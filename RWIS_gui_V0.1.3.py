...
class Mixin:
    ...
    def run_atmospheric():
            ...
            Mixin.gather_today()
            Mixin.gather_yesterday()
            AtmoSiteList_t, w = collection_today
            AtmoSiteList_y, u = collection_yesterday

            df_t = pd.DataFrame(AtmoSiteList_t) #Frame the site identifying info
            df_y = pd.DataFrame(AtmoSiteList_y)
            df_t.columns = ['Site','Local', 'Today\'s Issue'] #Name the Columns for site info
            df_t = df_t.drop('Local',1)
            ...
            df_y.set_index("Site", inplace =True)
                ...
                #Merge the data
            d={"left_only":"**NEW**", "right_only":"--resolved","both":"Persistent"}
            ...
            Atmospheric_Diff["Site"] = Atmospheric_Diff.index
            #Reorder columns
            ...
            return Atmospheric_Diff
        
    def run_surface():
            
            Mixin.gather_today()
            Mixin.gather_yesterday()
            w, SurfaceSiteList_t = collection_today
            u, SurfaceSiteList_y = collection_yesterday
            ...
            Surface_Diff["Site"] = Surface_Diff.index
            Surface_Diff = Surface_Diff[['Attn: Issue', 'Attn: Sensor' ,'Site','Issue_Today','Sensor_Today','Issue_Yesterday','Sensor_Yesterday','_merge']]
...
class StartPage(tk.Frame, Mixin):
    ...
    def __init__(self, parent, controller):
        ...
        button_T = tk.Button(self, text="Select Today's Report",command = Mixin.grab_datatoday(self))
        button_T.pack()
            
        button_T = tk.Button(self, text="Select Yesterday's Report",command= Mixin.grab_datayesterday(self))
        button_T.pack()
        
        button1 = tk.Button(self, text="Run Atmospheric Diff",command=lambda: controller.show_frame("Atmospheric_Report"))
        button1.pack()
...
class Atmospheric_Report(tk.Frame, Mixin):              
    ...
    def __init__(self, parent, controller):
        ...
        Atmospheric_Diff = Mixin.run_atmospheric()
        ...
        #TAG CONFIGURATION
        # tva.insert('', 'end', text='Diff', tags=('bg'))
        # tva.tag_configure('bg', background='yellow')
        #tva.tag_configure('fg', foreground='yellow')
        ...
        highlight_diff(Atmospheric_Diff)
...
class Surface_Report(tk.Frame, Mixin):     
    ...
    def __init__(self, parent, controller):
        ...
        Surface_Diff=Mixin.run_surface()
...
print('RWIS - Diff Finder, created by Joshua McMahon for Lumin8 Technologies. JAN 2022. V0.1.3') 
